...
//allocate gpu mem
	...
	if (!IsOldGpu)
	{
		...
		if (err != cudaSuccess)
		{
			printf("GPU %d, Ошибка выделения памяти L2: %s\n", CudaIndex, cudaGetErrorString(err));
			return false;
		}
		size = L2size;
		int maxL2 = 0;
		...
		if (err != cudaSuccess)
		{
			printf("GPU %d, ошибка cudaStreamSetAttribute: %s\n", CudaIndex, cudaGetErrorString(err));
			return false;
		}
	}
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка выделения памяти GpuOut: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка выделения памяти pKangs: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка выделения памяти Jumps1: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка выделения памяти Jumps3: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка выделения памяти JumpsList: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка выделения памяти DPTable: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка выделения памяти L1S2: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка выделения памяти LastPnts: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка выделения памяти dbg_buf: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка выделения памяти LoopedKangs: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
...
//jmp1
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка cudaMemcpy Jumps1: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	free(buf);
//jmp2
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка cudaMemcpy Jumps2: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	free(buf);
	...
	{
		free(jmp2_table);
		printf("GPU %d, ошибка cuSetGpuParams: %s!\r\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	free(jmp2_table);
//jmp3
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка cudaMemcpy Jumps3: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
	free(buf);

	printf("GPU %d: выделено %llu МБ, %d кенгуру. Режим OldGpu: %s\r\n", CudaIndex, total_mem / (1024 * 1024), KangCnt, IsOldGpu ? "Да" : "Нет");
	return true;
}
...
/* 
	...
	if (err != cudaSuccess)
	{
		printf("GPU %d, ошибка cudaMemcpy: %s\n", CudaIndex, cudaGetErrorString(err));
		return false;
	}
/**/
...
void RCGpuKang::Execute()
{
	...
	while (!StopFlag)
	{
		...
		if (err != cudaSuccess)
		{
			printf("GPU %d, ошибка CallGpuKernel: %s\r\n", CudaIndex, cudaGetErrorString(err));
			gTotalErrors++;
			break;
		}
		...
		{
			cnt = MAX_DP_CNT;
			printf("GPU %d, переполнение буфера DP GPU, некоторые точки потеряны, увеличьте значение DP!\r\n", CudaIndex);
		}
